...
sim_ordinal <- function(){
  ...
  for(j in seq_along(or_new_trt)){
    ...
    l_res[[j]] <- mclapply(1:n_sim, function(i){
      ...
      # dcast(d_trial[, .N, keyby = .(trt, outcome)], outcome ~ trt, value.var = "N")
      ...
      #   K = length(cats),
      #   P = 1,
      #   y = as.integer(d_trial$outcome),
      #   X = as.matrix(d_trial$x, ncol = 1, drop = F )
      # )
      # 
      # f3 <- m2$sample(
      #   ld, iter_warmup = 1000, iter_sampling = 2000,
      ...
      setcolorder(d_smry_rd, c("outcome", "rd_obs", "rd_mu_ord", "rd_mu_bin"))
      ...
      d_post <- melt(d_tmp, measure.vars = names(d_tmp))
      d_smry_ord_b <- d_post[, .(p_mu_bin = mean(value),
      ...
      d_tmp <- data.table(f4$draws(variables = c("b"), format = "matrix"))
      d_post <- melt(d_tmp, measure.vars = names(d_tmp))
      d_smry_bin_b <- d_post[, .(p_mu_bin = mean(value),
                             q_025 = quantile(value, prob = 0.025),
                             q_975 = quantile(value, prob = 0.975),
                             pr_lt_0 = mean(value < 0))]
...
#| echo: FALSE
#| label: fig-pwr_3_rd
#| fig-cap: 'Power curves characterising power by effect size (risk difference) showing impact when ordinal outcome for the primary outcome.'
#| fig.height: 4.5
#| fig.width: 4.5
#| fig-pos: H
