...
Within this simulation, probability thresholds of 0.96 and 0.35 were used for superiority and futility respectively with both comparing the difference in RMST relative to a zero reference point; superiority looking for extended RMST in the treatment group and futility looking for reduced RMST.
...
l_res <- qs::qread(file.path(output_dir_res, "20250402105647-sim-res.qs"))
...
@fig-pwr_1 shows the cumulative probability of superiority/futility by interim and by risk difference at day 360 with a maximum sample size of 3000 and two interim analyses.
...
l_res <- qs::qread(file.path(output_dir_res, "20250402125518-sim-res.qs"))
...
@fig-pwr_2 shows the cumulative probability of superiority/futility by interim and by risk difference at day 360 with a maximum sample size of 2500 and two interim analyses.
...
ggplot(
  ...
  geom_point(size = 0.5) +
  scale_color_discrete("RD (day 360)") +
  scale_x_continuous("", c(2000, 2250, 2500)) +
  scale_y_continuous(
    "Cumulative probability of decision", limits = c(0, 1),
    breaks = seq(0, 1, by = 0.1)) +
  theme_minimal() +
