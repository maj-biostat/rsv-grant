...
#| echo: FALSE
#| label: fig-cdf-1
#| fig-cap: 'Cumulative distribution of RSV event based on posterior means'
#| fig.height: 6
#| fig.width: 6
#| fig-pos: H
#| lightbox: true
...
d_fig <- d_sim_res[, .(id_sim, id_analy, N = N_enrol, rd_sim, scale0, scale1, shape)]
d_fig <- d_fig[N == 3000]

d_fig <- d_fig[sample(1:nrow(d_fig), size = 400, replace =  F)]

l_surv0 <- lapply(1:nrow(d_fig), function(i){
  1 / ( 1 + ( 0:360 / d_fig$scale0[i]  )^d_fig$shape[i] )  
})
d_fig[, surv0 := l_surv0]

l_surv1 <- lapply(1:nrow(d_fig), function(i){
  1 / ( 1 + ( 0:360 / d_fig$scale1[i]  )^d_fig$shape[i] )  
})
d_fig[, surv1 := l_surv1]

d_fig <- d_fig[, .(surv0 = l_surv0[[1]], surv1 = l_surv1[[1]]), by = .(id_sim, N)]
d_fig <- melt(d_fig, id.vars = c("id_sim", "N"))
d_fig[, trt := factor(fifelse(variable == "surv0", 0, 1))]
d_fig[, t_surv := 0:(.N-1), by = .(id_sim, N, trt)]
d_fig[, cdf := 1 - value]

d_fig[, grp := paste0(id_sim, ":", N, ":", trt)]

ggplot(d_fig[id_sim == 939], aes(x = t_surv, y = cdf, group = trt)) +
  geom_line(lwd = 0.3) +
  # scale_x_continuous("", c(2000, 2500, 3000)) +
  # scale_y_continuous(
  #   "Distribution of mean RD", 
